...
const SNENewton = (data: string[], x0, y0, e: number) => {
    ...
    while (counter < 100) {
        ...
        const preResult = multiply(inversedJacob, data.map(item => evaluate(item, {x: xPrev[0], y: xPrev[1]})));
...
const fromStrToNumber = (matrix: string[][], x: number, y: number): number[][] => matrix.map(item => item.map(el => evaluate(el, {
    x,
    y
})))
