...
const Regress = (data: string[], mode: number = 0) => {
    ...
    if (!mode) {
        ...
        for (let i = 0; i < matrixParametr.length; i++) {
            ...
            answer.push(det(newMatrix) / determ)
        }

    } else {
        const div: number = arrPow(matrix[0], 1) ** 2 - matrix[0].length * arrPow(matrix[0], 2);
        answer.push((arrPow(matrix[0], 1) * arrPow(matrix[1], 1) - matrix[0].length * matrix[0].reduce((accum, item, index) => {
            return accum + item * matrix[1][index]
        }, 0)) / div);
        answer.push((arrPow(matrix[0], 1) * matrix[0].reduce((accum, item, index) => {
            return accum + item * matrix[1][index]
